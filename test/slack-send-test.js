...
describe("slack-send", () => {
  ...
  it("should set an error if no webhook URL or token is provided", async () => {
    ...
    assert.include(
      fakeCore.setFailed.lastCall.firstArg.message,
      "Need to provide at least one botToken or webhook",
      "Error set specifying what env vars need to be set.",
    );
  });
  ...
  describe("using a bot token", () => {
    ...
    describe("happy path", () => {
      it("should send a message using the postMessage API", async () => {
        fakeCore.getInput
          .withArgs("slack-message")
          .returns("who let the dogs out?");
      ...
      it("should send payload-file-path values with replaced context variables", async () => {
        ...
        fakeCore.getInput
          ...
          .returns("./test/resources/valid-payload.json");
        fakeCore.getBooleanInput
      ...
      it("should send the same message to multiple channels", async () => {
        ...
        assert.oneOf(
          "C123456",
          [firstChatArgs.channel, secondChatArgs.channel],
          "First comma-separated channel provided to postMessage",
        );
        assert.oneOf(
          "C987654",
          [firstChatArgs.channel, secondChatArgs.channel],
          "Second comma-separated channel provided to postMessage",
        );
        assert.equal(
          firstChatArgs.text,
          "who let the dogs out?",
          "Correct message provided to postMessage with first comma-separated channel",
        );
        assert.equal(
          secondChatArgs.text,
          "who let the dogs out?",
          "Correct message provided to postMessage with second comma-separated channel",
        );
      });
    ...
    describe("sad path", () => {
      it("should set an error if payload cannot be JSON parsed", async () => {
        ...
        assert.include(
          fakeCore.setFailed.lastCall.firstArg.message,
          "Need to provide valid JSON",
          "Error set specifying JSON was invalid.",
        );
      });
      ...
      it("should fail if an invalid payload-file-path is provided", async () => {
        ...
        fakeCore.getInput
          .withArgs("payload-file-path")
          .returns("non-existing-path.json");
        ...
        assert.include(
          fakeCore.setFailed.lastCall.firstArg.message,
          "The payload-file-path may be incorrect. Failed to load the file: non-existing-path.json",
          "Error set specifying JSON was invalid.",
        );
      });
      ...
      it("should fail if a valid payload-file-path with an invalid JSON is provided", async () => {
        ...
        fakeCore.getInput
          .withArgs("payload-file-path")
          .returns("./test/resources/invalid-payload.json");
        ...
        assert.include(
          fakeCore.setFailed.lastCall.firstArg.message,
          "Need to provide valid JSON payload",
          "Error set specifying JSON was invalid.",
        );
      });
    ...
    describe("happy path", () => {
      ...
      describe("proxy config", () => {
        ...
        it("should use https proxy agent when proxy uses HTTP", async () => {
          ...
          await slackSend(fakeCore);
          assert(
            AxiosMock.post.calledWith(
              "https://someurl",
              payload,
              sinon.match.has("httpsAgent").and(sinon.match.has("proxy")),
            ),
          );
        });
        it("should use default axios config when no proxy set", async () => {
          await slackSend(fakeCore);
          assert(
            AxiosMock.post.calledWithExactly("https://someurl", payload, {}),
          );
        });
    ...
    describe("sad path", () => {
      it("should set an error if the POST to the webhook fails without a response", async () => {
        ...
        assert.include(
          ...
          "boom",
          "Error set to whatever axios reports as error.",
        );
      });
    });
  });
